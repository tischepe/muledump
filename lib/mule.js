...
function addfame($t, d) {
	...
	var $current_fame = $('<span>').addClass('current_fame').text('Current fame: ' + d.Account.Stats.Fame);
	var $total_fame = $('<span>').addClass('total_fame').text('total fame earned: ' + d.Account.Stats.TotalFame);
	$current_fame.appendTo($t);
	$total_fame.appendTo($t);
}

function addgold($t, d) {
	if (d.Account.Credits == 0) return;
	var $current_gold = $('<span>').addClass('gold').text(d.Account.Credits);
	$current_gold.appendTo($t);
}

function addguild($t, d) {
	switch (+d.Account.Guild.Rank) {
		case 0:
			guildRankClass = 'guild_initiate'
			break;
		case 10:
			guildRankClass = 'guild_member'
			break;
		case 20:
			guildRankClass = 'guild_officer'
			break;
		case 30:
			guildRankClass = 'guild_leader'
			break;
		case 40:
			guildRankClass = 'guild_founder'
			break;
		default:
			console.log('unknown guild rank');
		}

	var $guild_rank = $('<span>').addClass(guildRankClass).text(d.Account.Guild.Name);;
	$guild_rank.appendTo($t);
}

function addtoken($t, d) {
	if (d.Account.FortuneToken == 0) return;
	var $current_token = $('<span>').addClass('token').text(d.Account.FortuneToken);
	$current_token.appendTo($t);
}
...
/*	if (this.data && compare(d, this.data.query.results.Chars)) {
	...
	if (this.opt('fame') || this.opt('guild') || this.opt('gold') || this.opt('token')) {
		var $accountStats = $('<div class="account_stats">');
		if (this.opt('fame')) {
			addfame($accountStats, d);
		}
		if (this.opt('guild') && ('Guild' in d.Account)) {
			addguild($accountStats, d);
		}
		if (this.opt('gold')) {
			addgold($accountStats, d);
		}
		if (this.opt('token')) {
			addtoken($accountStats, d);
		}
		if( !$accountStats.is(':empty') ) {
			$accountStats.prepend($('<hr>'));
			$accountStats.appendTo(this.dom);
		}
	}
